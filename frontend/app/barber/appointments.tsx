...
export default function BarberAppointmentsScreen() {
  ...
  const handleStatusChange = (appointment: Appointment, newStatus: string) => {
    ...
    Alert.alert(
      'Confirmar Cambio',
      `¿Cambiar el estado de la cita a ${statusTexts[newStatus as keyof typeof statusTexts]}?`,
      [
    ...
    switch (filter) {
      case 'today':
        return appointments.filter(appointment => 
          new Date(appointment.scheduled_at).toDateString() === today
        );
      case 'tomorrow':
        return appointments.filter(appointment => 
          new Date(appointment.scheduled_at).toDateString() === tomorrow
        );
      case 'upcoming':
        return appointments.filter(appointment => 
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.statsContainer}>
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>{getStatsForFilter('today')}</Text>
          <Text style={styles.statLabel}>Hoy</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>{getStatsForFilter('pending')}</Text>
          <Text style={styles.statLabel}>Pendientes</Text>
        </View>
        <View style={styles.statCard}>
          <Text style={styles.statNumber}>{getStatsForFilter('confirmed')}</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtext}>
              {filter === 'today' ? 'No tienes citas programadas para hoy' : 
               filter === 'tomorrow' ? 'No tienes citas programadas para mañana' :
